...
BOOL CALLBACK WindowNameFilter(HWND hWnd, LPARAM lParam)
{
    ...
    if (GetWindowTextA(hWnd, szWindowTitle, sizeof(szWindowTitle)))
    {
        ...
        if (!windowTitle.empty())
        {
            ...
            {
                if (SharedUtil::FindStringIC(windowTitle, std::string(IterWindowName)))
                {
                    char szBanReason[256];
                    sprintf(szBanReason, XorStr("%s Detected").c_str(), IterWindowName);
...
void SharedChecks::CheckProcessList(void (*found_process)(char* szProcessName))
{
    std::vector<std::string> vBlacklistProcesses = {_("x96dbg"),
                                                    ...
                                                    XorStr("dnSpy"),
                                                    _("dnSpy32"),
                                                    XorStr("cheatengine-x86_64-SSE4-AVX2"),
                                                    _("cheatengine-x86_64"),
                                                    XorStr("cheatengine-i386"),
                                                    _("ProcessHacker"),
                                                    XorStr("HTTPDebugger"),
                                                    XorStr("HTTPDebuggerPro"),
                                                    _("wireshark"),
                                                    XorStr("die")};
