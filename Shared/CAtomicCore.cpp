...
std::string CAtomicCore::GetMD5Hash(const std::string& data)
{
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    BYTE       hash[16];
    ...
    {
        SharedUtil::AddDebugLog("CryptAcquireContext failed");
        return {};
    }

    if (!CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash))
    {
        SharedUtil::AddDebugLog("CryptCreateHash failed");
    ...
    {
        SharedUtil::AddDebugLog("CryptHashData failed");
    ...
    if (CryptGetHashParam(hHash, HP_HASHVAL, hash, &hashLen, 0))
    {
        for (DWORD i = 0; i < hashLen; ++i)
            sprintf_s(hexDigest + i * 2, 3, "%02x", hash[i]);
    }
    else
    {
        SharedUtil::AddDebugLog("CryptGetHashParam failed");
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        return {};
    }
