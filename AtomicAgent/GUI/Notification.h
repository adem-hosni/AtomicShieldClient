...
namespace image
{
    ...
    IDirect3DTexture9* loading_notif = nullptr;
}
namespace font
{
...
namespace ImGui
{
    ...
    NOTIFY_INLINE VOID RenderNotifications()
    {
        ...
        for (auto i = 0; i < notifications.size(); i++)
        {
            ...
            PushStyleColor(ImGuiCol_WindowBg, GetColorU32(c::notif));
            PushStyleColor(ImGuiCol_Border, GetColorU32(c::notif));
            ...
            Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);
            {
                ...
                blur::add_blur(ImGui::GetBackgroundDrawList(), pos, ImVec2(pos.x + region.x + 20, pos.y + region.y + 10), 1.f);

              //  GetForegroundDrawList()->AddRectFilled(pos + ImVec2(20, region.y - 16), pos + ImVec2(region.x, region.y - 10), GetColorU32(c::notif), 30.f);

               // if (image::loading_notif == nullptr) D3DXCreateTextureFromFileInMemoryEx(g_pd3dDevice, loading_notif_icon, sizeof(dota_bg_image), 600, 600, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &image::loading_notif); ImGui::GetWindowDrawList()->AddImageRounded(image::loading_notif, ImVec2(pos.x + 14, pos.y + 21), ImVec2(pos.x + 34, pos.y + 41), ImVec2(0, 0), ImVec2(1, 1), ImGui::GetColorU32(c::circle_icon) /*color*/, 20 /*rounding*/);
                ...
                PushTextWrapPos(vp_size.x + 100);

               // PushFont(font::icon);
                SetCursorPosX(GetCursorPosX() + 33);
                if (default_title == "Success") TextColored(c::circle_icon, "Succes injection");
               ...
               // PopFont();
                ...
                {
                    TextColored(ImColor(GetColorU32(c::text_notif)), content);
                }
