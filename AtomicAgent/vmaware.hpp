...
private:
    ...
    static constexpr u16 base_technique_count =
        NO_MEMO;            // original technique count, constant on purpose (can also be used as a base count value if custom techniques are added)
    ...
    static constexpr bool SHORTCUT = true;            // macro for whether VM::core::run_all() should take a shortcut by skipping the rest of the techniques if
                                                      // the threshold score is already met
...
// likely and unlikely macros
    ...
    struct cpu
    {
        ...
        struct leaf
        {
            static constexpr u32 func_ext = 0x80000000, proc_ext = 0x80000001, brand1 = 0x80000002, brand2 = 0x80000003, brand3 = 0x80000004,
                                 hypervisor = 0x40000000, amd_easter_egg = 0x8fffffff;
        };
        ...
        [[nodiscard]] static bool is_celeron(const stepping_struct steps)
        {
            ...
            return (steps.model == celeron_model && steps.family == celeron_family && steps.extmodel == celeron_extmodel);
        }
...
#endif
                bhyve = "bhyve bhyve ",
            bhyve2 = "BHyVE BHyVE ", kvm_hyperv = "Linux KVM Hv", qemu = "TCGTCGTCGTCG", hyperv = "Microsoft Hv", parallels = " prl hyperv ",
            parallels2 = " lrpepyh  vr", vmware = "VMwareVMware", vbox = "VBoxVBoxVBox", xen = "XenVMMXenVMM", acrn = "ACRNACRNACRN", qnx = " QNXQVMBSQG ",
            qnx2 = "QXNQSBMV", nvmm = "___ NVMM ___", openbsd_vmm = "OpenBSDVMM58", intel_haxm = "HAXMHAXMHAXM", unisys = "UnisysSpar64", lmhs = "SRESRESRESRE",
            jailhouse = "Jailhouse\0\0\0", apple_vz = "Apple VZ", intel_kgt = "EVMMEVMMEVMM", barevisor = "Barevisor!\0\0", hyperplatform = "PpyH",
            minivisor = "MiniVisor\0\0\0", intel_tdx = "IntelTDX    ", lkvm = "LKVMLKVMLKVM", neko = "Neko Project", noir = "NoirVisor ZT";
        ...
        if (brand_str == kvm_hyperv)
        {
            return core::add(brands::KVM_HYPERV);
        }
        if (brand_str == parallels)
        {
            return core::add(brands::PARALLELS);
        }
        if (brand_str == parallels2)
        {
            return core::add(brands::PARALLELS);
        }
        if (brand_str == bhyve)
        ...
        if (brand_str == nvmm)
        {
            return core::add(brands::NVMM);
        }
        if (brand_str == openbsd_vmm)
        {
            return core::add(brands::BSD_VMM);
        }
        if (brand_str == intel_haxm)
        {
            return core::add(brands::INTEL_HAXM);
        }
        if (brand_str == unisys)
        {
            return core::add(brands::UNISYS);
        }
        if (brand_str == lmhs)
        {
            return core::add(brands::LMHS);
        }
        if (brand_str == jailhouse)
        {
            return core::add(brands::JAILHOUSE);
        }
        if (brand_str == intel_kgt)
        {
            return core::add(brands::INTEL_KGT);
        }
        if (brand_str == barevisor)
        {
            return core::add(brands::BAREVISOR);
        }
        if (brand_str == minivisor)
        {
            return core::add(brands::MINIVISOR);
        }
        if (brand_str == intel_tdx)
        {
            return core::add(brands::INTEL_TDX);
        }
        if (brand_str == lkvm)
        {
            return core::add(brands::LKVM);
        }
        if (brand_str == neko)
        {
            return core::add(brands::NEKO_PROJECT);
        }
        if (brand_str == noir)
        {
            return core::add(brands::NOIRVISOR);
        }
...
public:
    ...
    static bool is_cached(const u16 technique_macro) { return cache_keys.test(technique_macro); }

    static data_t cache_fetch(const u16 technique_macro) { return cache_table.at(technique_macro); }
    ...
    static bool all_present()
    {
        ...
        {
            return (!cache_keys.test(VMWARE_DMESG) && !cache_keys.test(PORT_CONNECTORS) && !cache_keys.test(ACPI_TEMPERATURE) && !cache_keys.test(LSHW_QEMU) &&
    ...
    struct brand
    {
        ...
        static void store(const std::string& p_brand) { brand_cache = p_brand; }

        static bool is_cached() { return (!brand_cache.empty()); }
    };
...
struct util
{
    ...
    [[nodiscard]] static bool find(const std::string& base_str, const char* keyword) noexcept { return (base_str.find(keyword) != std::string::npos); };
    ...
    [[nodiscard]] static u32 get_disk_size()
    {
        ...
        size = static_cast<u32>((stat.f_blocks * stat.f_frsize) / GB);
            ...
            if (GetDiskFreeSpaceExW(L"C:",                          // Drive or directory path (use wide character string)
    ...
    [[nodiscard]] static std::unordered_set<std::string> get_running_process_names()
    {
        ...
        do
        {
            ...
            status = pNtQSI(SystemProcessInformation, buffer.get(), bufSize, &needed);
        ...
        for (BYTE* cur = buffer.get();;)
        {
            ...
            if (pi->ImageName.Length > 0 && pi->ImageName.Buffer)
            {
                ...
                const int utf8ByteCount = WideCharToMultiByte(CP_UTF8, 0, pi->ImageName.Buffer, static_cast<int>(wideCharCount), nullptr, 0, nullptr, nullptr);
                ...
                if (utf8ByteCount > 0)
                {
                    ...
                    WideCharToMultiByte(CP_UTF8, 0, pi->ImageName.Buffer, static_cast<int>(wideCharCount), &name[0], utf8ByteCount, nullptr, nullptr);
     ...
     * @note Hyper-V's presence on a host system can set certain hypervisor-related CPU flags that may appear similar to those in a virtualized environment,
     * which can make it challenging to differentiate between an actual Hyper-V virtual machine (VM) and a host system with Hyper-V enabled. This can lead to
     * false conclusions, where the system might mistakenly be identified as running in a Hyper-V VM, when in reality, it's simply the host system with Hyper-V
     * features active. This check aims to distinguish between these two cases by identifying specific CPU flags and hypervisor-related artifacts that are
     * indicative of a Hyper-V VM rather than a host system with Hyper-V enabled.
    ...
    [[nodiscard]] static hyperx_state hyper_x()
    {
        ...
        return HYPERV_UNKNOWN_VM;
            ...
            // 0x40000003 on EBX indicates the flags that a parent partition specified to create a child partition
             ...
             * Essentially, it indicates that the hypervisor is managing the VM and that the VM is not running directly on hardware but rather in a virtualized
             * environment
             */
            auto eax = []() -> u32
            {
    ...
    public:
            ...
            if (bios_data)
            {
                ...
                if (sysinfo)
                {
                    ...
                    if (sysinfo->Header.length > 0x08)
                    {
                        ...
                        _snprintf_s(uuid, max_uuid_size, static_cast<size_t>(max_uuid_size) - 1,
                                    "%02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X", sysinfo->UUID[0], sysinfo->UUID[1],
                                    sysinfo->UUID[2], sysinfo->UUID[3], sysinfo->UUID[4], sysinfo->UUID[5], sysinfo->UUID[6], sysinfo->UUID[7],
                                    sysinfo->UUID[8], sysinfo->UUID[9], sysinfo->UUID[10], sysinfo->UUID[11], sysinfo->UUID[12], sysinfo->UUID[13],
        ...
        // get product family
        const std::string get_family() const { return family_; }
        ...
        // get product name
        const std::string get_productname() const { return productname_; }

        // get BIOS serial number
        const std::string get_serialnumber() const { return serialnumber_; }

        // get SKU / system configuration
        const std::string get_sku() const { return sku_; }
    ...
    {
        const std::map<DWORD, u8> windowsVersions = {{static_cast<DWORD>(6002), static_cast<u8>(6)},            // windows vista
                                                     {static_cast<DWORD>(7601), static_cast<u8>(7)},   {static_cast<DWORD>(9200), static_cast<u8>(8)},
                                                     {static_cast<DWORD>(9600), static_cast<u8>(8)},   {static_cast<DWORD>(10240), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(10586), static_cast<u8>(10)}, {static_cast<DWORD>(14393), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(15063), static_cast<u8>(10)}, {static_cast<DWORD>(16299), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(17134), static_cast<u8>(10)}, {static_cast<DWORD>(17763), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(18362), static_cast<u8>(10)}, {static_cast<DWORD>(18363), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(19041), static_cast<u8>(10)}, {static_cast<DWORD>(19042), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(19043), static_cast<u8>(10)}, {static_cast<DWORD>(19044), static_cast<u8>(10)},
                                                     {static_cast<DWORD>(19045), static_cast<u8>(10)}, {static_cast<DWORD>(22000), static_cast<u8>(11)},
                                                     {static_cast<DWORD>(22621), static_cast<u8>(11)}, {static_cast<DWORD>(22631), static_cast<u8>(11)},
    ...
    [[nodiscard]] static std::wstring GetLastErrorString()
    {
        ...
        size_t      size = FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, error, 0,
    ...
    static void GetFunctionAddresses(const HMODULE hModule, const char* names[], void** functions, size_t count)
    {
        ...
        const PIMAGE_NT_HEADERS       ntHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<BYTE*>(hModule) + dosHeader->e_lfanew);
...
[[nodiscard]] static bool vmid()
{
    ...
    return false;
#else
        return (cpu::vmid_template(0) || cpu::vmid_template(cpu::leaf::hypervisor) ||            // 0x40000000
...
[[nodiscard]] static bool cpu_brand()
{
    ...
    return false;
        ...
        static const std::array<KeywordCheck, 10> checks = {{{"qemu", std::regex("qemu", std::regex::icase | std::regex::optimize)},
...
/**
 * @brief Check if there are only 1 or 2 threads, which is a common pattern in VMs with default settings (nowadays physical CPUs should have at least 4 threads
 * for modern CPUs
...
[[nodiscard]] static bool mac_address_check()
{
...
#ifdef __VMAWARE_DEBUG__
    std::stringstream ss;
    ss << std::setw(2) << std::setfill('0') << std::hex << static_cast<i32>(mac[0]) << ":" << static_cast<i32>(mac[1]) << ":" << static_cast<i32>(mac[2])
       << ":XX:XX:XX";
    ...
    */
    ...
    auto compare = [mac](u8 mac1, u8 mac2, u8 mac3) noexcept -> bool { return (mac[0] == mac1 && mac[1] == mac2 && mac[2] == mac3); };

    // Check for known virtualization MAC address prefixes
    if (compare(0x08, 0x00, 0x27))
        return core::add(brands::VBOX);

    if (compare(0x00, 0x0C, 0x29) || compare(0x00, 0x1C, 0x14) || compare(0x00, 0x50, 0x56) || compare(0x00, 0x05, 0x69))
    {
        return core::add(brands::VMWARE);
    }
...
[[nodiscard]] static bool dll_check()
{
    ...
    return false;
        ...
        static constexpr struct
        {
            const char* dll_name;
            const char* brand;
        } dll_checks[] = {{"sbiedll.dll", brands::SANDBOXIE}, {"pstorec.dll", brands::CWSANDBOX}, {"vmcheck.dll", brands::VPC},
                          {"cmdvrt32.dll", brands::COMODO},   {"cmdvrt64.dll", brands::COMODO},   {"cuckoomon.dll", brands::CUCKOO},
...
[[nodiscard]] static bool vm_files()
{
    ...
    return false;
        ...
        constexpr std::array<const char*, 12> vmwareFiles = {{"drivers\\Vmmouse.sys", "drivers\\Vmusbmouse.sys", "drivers\\vm3dgl.dll", "drivers\\vmdum.dll",
                                                              "drivers\\VmGuestLibJava.dll", "drivers\\vm3dver.dll", "drivers\\vmtray.dll",
                                                              "drivers\\VMToolsHook.dll", "drivers\\vmGuestLib.dll", "drivers\\vmhgfs.dll",
                                                              "vm3dum64_loader.dll", "vm3dum64_10.dll"}};

        constexpr std::array<const char*, 17> vboxFiles = {{"drivers\\VBoxMouse.sys", "drivers\\VBoxGuest.sys", "drivers\\VBoxSF.sys", "drivers\\VBoxVideo.sys",
                                                            "vboxoglpackspu.dll", "vboxoglpassthroughspu.dll", "vboxservice.exe", "vboxoglcrutil.dll",
                                                            "vboxdisp.dll", "vboxhook.dll", "vboxmrxnp.dll", "vboxogl.dll", "vboxtray.exe", "VBoxControl.exe",
                                                            "vboxoglerrorspu.dll", "vboxoglfeedbackspu.dll", "vboxoglarrayspu.dll"}};

        constexpr std::array<const char*, 10> kvmFiles = {{"drivers\\balloon.sys", "drivers\\netkvm.sys", "drivers\\pvpanic.sys", "drivers\\viofs.sys",
                                                           "drivers\\viogpudo.sys", "drivers\\vioinput.sys", "drivers\\viorng.sys", "drivers\\vioscsi.sys",
                                                           "drivers\\vioser.sys", "drivers\\viostor.sys"}};

        constexpr std::array<const char*, 7> parallelsFiles = {{"drivers\\prleth.sys", "drivers\\prlfs.sys", "drivers\\prlmouse.sys", "drivers\\prlvideo.sys",
                                                                "drivers\\prltime.sys", "drivers\\prl_pv32.sys", "drivers\\prl_paravirt_32.sys"}};

        constexpr std::array<const char*, 2> vpcFiles = {{"drivers\\vmsrvc.sys", "drivers\\vpc-s3.sys"}};
...
[[nodiscard]] static bool vbox_default_specs()
{
    ...
    return false;
        ...
        if ("arch" == distro || "opensuse" == distro || "redhat" == distro || "gentoo" == distro || "fedora" == distro || "debian" == distro)
        {
            return ((8 == disk) && (1 == ram));
        }
...
/* GPL */ \
        ...
        /* GPL */
        /* GPL */ if (compare("InsideTm") || compare("TU-4NH09SMCG1HC"))
        {            // anubis
            /* GPL */ debug("COMPUTER_NAME: detected Anubis");
            /* GPL */ return core::add(brands::ANUBIS);
/* GPL */         }
/* GPL */
/* GPL */ if (compare("klone_x64-pc") || compare("tequilaboomboom"))
{            // general
...
/* GPL */ \
    ...
    /* GPL */ TCHAR szPath[MAX_PATH] = _T("");
    /* GPL */
    /* GPL */ const TCHAR* szDirectories[] = {/* GPL */ _T("qemu-ga"),                      // QEMU guest agent.
...
/* GPL */ \
    ...
    /* GPL */ DWORD           i;
    /* GPL */
    /* GPL */ constexpr GUID GUID_DEVCLASS_DISKDRIVE = {/* GPL */ 0x4d36e967L, 0xe325, 0x11ce,
...
/* GPL */ if (GetLastError() != NO_ERROR && GetLastError() != ERROR_NO_MORE_ITEMS)
    return false;
/* GPL */
/* GPL */ return false;
/* GPL */ \
...
[[nodiscard]] static bool vm_processes()
{
    ...
    if (runningProcesses.count("vm3dservice.exe") || runningProcesses.count("VGAuthService.exe") || runningProcesses.count("vmtoolsd.exe"))
    {
        debug("VM_PROCESSES: Detected VMware process.");
        return core::add(brands::VMWARE);
    }

    if (runningProcesses.count("vdagent.exe") || runningProcesses.count("vdservice.exe") || runningProcesses.count("qemuwmi.exe"))
    {
        debug("VM_PROCESSES: Detected QEMU process.");
        return core::add(brands::QEMU);
    }

    if (runningProcesses.count("looking-glass-host.exe") || runningProcesses.count("VDDSysTray.exe"))
    {
...
[[nodiscard]] static bool linux_user_host()
{
    ...
    return ((strcmp(username, "liveuser") == 0) && (strcmp(hostname, "localhost-live") == 0));
#endif
}
...
[[nodiscard]] static bool gamarue()
{
    ...
    lRes = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion", 0, KEY_QUERY_VALUE, &hKey);

    if (lRes != ERROR_SUCCESS)
        return false;

    lRes = RegQueryValueExA(hKey, "ProductId", nullptr, nullptr, reinterpret_cast<LPBYTE>(buffer), &dwSize);
    ...
    constexpr TargetPattern targets[] = {
        {"55274-640-2673064-23950", "JOEBOX"}, {"76487-644-3177037-23510", "CWSANDBOX"}, {"76487-337-8429955-22614", "ANUBIS"}};
...
[[nodiscard]] static bool mssmbios()
{
    ...
    {
        LSTATUS ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Services\\mssmbios\\Data", 0, KEY_READ, &hk);
        ...
        ret = RegQueryValueExA(hk, config.value_name, nullptr, &type, reinterpret_cast<LPBYTE>(buffer), &length);
        ...
        {
            buffer[i] = (buffer[i] >= 'a' && buffer[i] <= 'z') ? static_cast<char>(buffer[i] - 32) : buffer[i];
        }
...
[[nodiscard]] static bool mutex()
{
    ...
    if (supMutexExist("Sandboxie_SingleInstanceMutex_Control") || supMutexExist("SBIE_BOXED_ServiceInitComplete_Mutex1"))
    {
...
[[nodiscard]] static bool odd_cpu_threads()
{
    ...
    auto old_microarchitecture = [&steps]() -> bool
    {
        ...
        {
            if (steps.family == arch.at(FAMILY) && steps.extmodel == arch.at(EXTMODEL) && steps.model == arch.at(MODEL))
            {
                return true;
            }
        }
...
[[nodiscard]] static bool intel_thread_mismatch()
{
    ...
    std::map<const char*, int, CStrComparator> thread_database = {// i3 series
                                                                  {"i3-1000G1", 4},
                                                                  {"i3-1000G4", 4},
                                                                  {"i3-1000NG4", 4},
                                                                  {"i3-1005G1", 4},
...
[[nodiscard]] static bool xeon_thread_mismatch()
{
    ...
    std::map<const char*, int, CStrComparator> thread_database = {// Xeon D
                                                                  {"D-1518", 8},
                                                                  {"D-1520", 8},
                                                                  {"D-1521", 8},
                                                                  {"D-1527", 8},
...
[[nodiscard]] static bool nettitude_vm_memory()
{
    ...
    const wchar_t* resource_keys[] = {L"Hardware\\ResourceMap\\System Resources\\Physical Memory", L"Hardware\\ResourceMap\\System Resources\\Reserved",
...
[[nodiscard]] static bool hyperv_hostname()
{
    ...
    if (len < 8)
        return false;            // "fv-az0-0" is minimum length

    // Check prefix "fv-az"
    if (!(hostname[0] == 'f' && hostname[1] == 'v' && hostname[2] == '-' && hostname[3] == 'a' && hostname[4] == 'z'))
    {
        return false;
    }
    ...
    if (hyphen_pos == std::string::npos || hyphen_pos <= 5 || hyphen_pos >= len - 1)
    {
        return false;
    }
...
[[nodiscard]] static bool general_hostname()
{
    ...
    auto cmp = [&](const char* str2) -> bool { return (hostname == str2); };

    if (cmp("Sandbox") || cmp("Maltest") || cmp("Malware") || cmp("malsand") || cmp("ClonePC"))
    {
        return true;
    }
...
[[nodiscard]] static bool screen_resolution()
{
    ...
    if ((horiz == 1024 && verti == 768) || (horiz == 800 && verti == 600) || (horiz == 640 && verti == 480))
    {
        return true;
    }
...
[[nodiscard]] static bool bluestacks()
{
    ...
    return false;
#else
    if (util::exists("/mnt/windows/BstSharedFolder") || util::exists("/sdcard/windows/BstSharedFolder"))
    {
...
[[nodiscard]] static bool kvm_bitmask()
{
    ...
    if (!((eax == 0x40000001) && (ebx == 0x4b4d564b) && (ecx == 0x564b4d56) && (edx == 0x4d)))
    {
        return false;
    }
    ...
    if ((eax & (1 << 8)) && (((eax >> 13) & 0b1111111111) == 0) && ((eax >> 24) == 0))
    {
        return core::add(brands::KVM);
    }
...
[[nodiscard]] static bool dmi_scan()
{
    ...
    */
    ...
    constexpr std::array<const char*, 7> dmi_array{"/sys/class/dmi/id/bios_vendor",       "/sys/class/dmi/id/board_name",     "/sys/class/dmi/id/board_vendor",
                                                   "/sys/class/dmi/id/chassis_asset_tag", "/sys/class/dmi/id/product_family", "/sys/class/dmi/id/product_sku",
...
[[nodiscard]] static bool wsl_proc_subdir()
{
    ...
    if (util::exists(osrelease) && util::exists(version))
    {
        ...
        if ((util::find(osrelease_content, "WSL") || util::find(osrelease_content, "Microsoft")) &&
...
[[nodiscard]] static bool driver_names()
{
    ...
    typedef NTSTATUS(__stdcall * NtQuerySystemInformationFn)(ULONG SystemInformationClass, PVOID SystemInformation, ULONG SystemInformationLength,
                                                             PULONG ReturnLength);

    typedef NTSTATUS(__stdcall * NtAllocateVirtualMemoryFn)(HANDLE ProcessHandle, PVOID * BaseAddress, ULONG_PTR ZeroBits, PSIZE_T RegionSize,
                                                            ULONG AllocationType, ULONG Protect);

    typedef NTSTATUS(__stdcall * NtFreeVirtualMemoryFn)(HANDLE ProcessHandle, PVOID * BaseAddress, PSIZE_T RegionSize, ULONG FreeType);
    ...
    for (ULONG i = 0; i < pSystemModuleInfoEx->NumberOfModules; ++i)
    {
        ...
        if (strstr(driverPath, "VBoxGuest") || strstr(driverPath, "VBoxMouse") || strstr(driverPath, "VBoxSF"))
        {
        ...
        if (strstr(driverPath, "vmusbmouse") || strstr(driverPath, "vmmouse") || strstr(driverPath, "vmmemctl"))
        {
...
[[nodiscard]] static bool disk_serial_number()
{
    ...
    auto is_vbox_serial = [](const char* str, size_t len) -> bool
    {
        ...
        if (len != 19)
            return false;
        if ((str[0] != 'V' && str[0] != 'v') || (str[1] != 'B' && str[1] != 'b'))
            return false;
    ...
    for (DWORD drive = 0; drive < MAX_PHYSICAL_DRIVES; drive++)
    {
        ...
        HANDLE hDevice = CreateFileW(path, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
        ...
        if (serialOffset > 0 && serialOffset < header.Size)
        {
            ...
            const size_t copyLen = (serialLen < sizeof(upperSerial)) ? serialLen : sizeof(upperSerial) - 1;
...
[[nodiscard]] static bool port_connectors()
{
    ...
    return false;
#else
    const GUID GUID_DEVCLASS_PORTCONNECTOR = {0x4d36e978, 0xe325, 0x11ce, {0xbf, 0xc1, 0x08, 0x00, 0x2b, 0xe1, 0x03, 0x18}};

    HDEVINFO hDevInfo = SetupDiGetClassDevsW(&GUID_DEVCLASS_PORTCONNECTOR, nullptr, nullptr, DIGCF_PRESENT);
...
[[nodiscard]] static bool ivshmem()
{
    ...
    return false;
#else
    const GUID GUID_IVSHMEM_IFACE = {0xdf576976, 0x569d, 0x4672, {0x95, 0xa0, 0xf5, 0x7e, 0x4e, 0xa0, 0xb2, 0x10}};

    HDEVINFO hDevInfo = SetupDiGetClassDevsW(&GUID_IVSHMEM_IFACE, nullptr, nullptr, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    if (hDevInfo == INVALID_HANDLE_VALUE)
    {
        return true;
    }
...
[[nodiscard]] static bool gpu_vm_strings()
{
    ...
    static constexpr std::array<VMGpuInfo, 8> vm_gpu_names = {{{L"VMware SVGA 3D", brands::VMWARE},
    ...
    static const VMGpuInfo vm_gpu_names[8] = {{L"VMware SVGA 3D", brands::VMWARE},
                                              {L"VirtualBox Graphics Adapter", brands::VBOX},
                                              {L"QXL GPU", brands::KVM},
                                              {L"VirGL 3D", brands::QEMU},
                                              {L"Microsoft Hyper-V Video", brands::HYPERV},
...
[[nodiscard]] static bool gpu_capabilities()
{
    ...
    CreateDXGIFactory2_t CreateDXGIFactory2_ = (CreateDXGIFactory2_t)RuntimeImportResolver::ResolveFunction("dxgi.dll", "CreateDXGIFactory2");
    if (FAILED(CreateDXGIFactory2_(0, IID_PPV_ARGS(&factory))))
    {
    ...
    if (warpMod)
    {
        ...
        if (SUCCEEDED(factory->CreateSoftwareAdapter(warpMod, &swAdapter)))
        {
            ...
            if (SUCCEEDED(swAdapter.As(&swAdapter1)))
            {
                ...
                if (SUCCEEDED(swAdapter1->GetDesc1(&swDesc)) && (swDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE))
                {
...
[[nodiscard]] static bool vm_devices()
{
    ...
    if ((handle1 != INVALID_HANDLE_VALUE) || (handle2 != INVALID_HANDLE_VALUE) || (handle3 != INVALID_HANDLE_VALUE) || (handle4 != INVALID_HANDLE_VALUE))
    {
        vbox = true;
    }
...
timer()
{
    ...
    tscSyncDetected = [&]() noexcept -> bool
    {
        ...
        for (int i = 0; i < tscIterations; ++i)
        {
            ...
            newAct.sa_sigaction = static_cast<sa_sigaction_fn>([](int, siginfo_t*, void*) { siglongjmp(jumpBuf, 2); });
            ...
            const u64 diff = (tscCore2 > tscCore1) ? (tscCore2 - tscCore1) : (tscCore1 - tscCore2);
    ...
    #ifdef __VMAWARE_DEBUG__
        debug("TIMER: TSC sync check", " - Core1: ", tscCore1, " Core2: ", tscCore2, " Delta: ", tscCore2 - tscCore1, " (Threshold: <", tscSyncDiffThreshold,
              ')');
    #endif
    ...
    for (int i = 0; i < SAMPLE_COUNT; ++i)
    {
        ...
        samples[i] = (heapCost > 0) ? ((closeCost * SCALE_FACTOR) / heapCost) : UINT64_MAX;
    }
    ...
    const NTSTATUS status = CallNtPowerInformation(ProcessorInformation, nullptr, 0, ppi.data(), sizeof(PROCESSOR_POWER_INFORMATION) * procCount);
...
[[nodiscard]] static bool sys_qemu_dir()
{
    ...
    if (util::is_directory(module_path.c_str()) && util::is_directory(firmware_path.c_str()) && util::exists(module_path.c_str()) &&
...
[[nodiscard]] static bool lshw_qemu()
{
    ...
    return false;
#else
    if (!(util::exists("/usr/bin/lshw") || util::exists("/bin/lshw") || util::exists("/usr/sbin/lshw")))
    {
    ...
    static constexpr const char* patterns[] = {"QEMU PCIe Root port", "QEMU XHCI Host Controller", "QEMU DVD-ROM", "QEMU QEMU USB Tablet"};
    bool                         seen[sizeof(patterns) / sizeof(patterns[0])] = {false};
    int                          score = 0;

    size_t max_pat = 0;
    for (auto& p : patterns)
        max_pat = std::max(max_pat, strlen(p));
    std::string overlap;
...
[[nodiscard]] static bool hyperv_query()
{
    ...
    typedef NTSTATUS(__stdcall * FN_NtQuerySystemInformation)(SYSTEM_INFORMATION_CLASS SystemInformationClass, PVOID SystemInformation,
...
[[nodiscard]] static bool bad_pools()
{
    ...
    typedef enum _SYSTEM_INFORMATION_CLASS
    {
        ...
        SystemTimeAdjustmentInformation,                  // q: SYSTEM_QUERY_TIME_ADJUST_INFORMATION; s: SYSTEM_SET_TIME_ADJUST_INFORMATION (requires
    ...
    typedef NTSTATUS(__stdcall * NtQuerySystemInformationFunc)(SYSTEM_INFORMATION_CLASS SystemInformationClass, PVOID SystemInformation,
                                                               ULONG SystemInformationLength, PULONG ReturnLength);
...
[[nodiscard]] static bool amd_sev()
{
    ...
    if (result & (static_cast<unsigned long long>(1) << 2))
    {
        return core::add(brands::AMD_SEV_SNP);
    }
    else if (result & (static_cast<unsigned long long>(1) << 1))
    {
...
[[nodiscard]] static bool amd_thread_mismatch()
{
    ...
    constexpr std::array<std::pair<const char*, int>, 559> amd_thread_database = {
        {{"3015ce ", 4},
         ...
         {"ai max+ 395 ", 32},
         {"ai max+ pro 395 ", 32},
         {"athlon  silver 3050c ", 2},            // there's an extra space in the AMD specifications for some reason, which I assume it's a typo. I added the
                                                  // fixed and typo'd version just in case.
...
[[nodiscard]] static bool virtual_registry()
{
    ...
    const bool mismatch =
        (pObjectName->Name.Length != expectedName.Length) || (memcmp(pObjectName->Name.Buffer, expectedName.Buffer, expectedName.Length) != 0);
...
[[nodiscard]] static bool firmware_scan()
{
    ...
    constexpr const char* targets[] = {"Parallels Software International",
                                       "Parallels(R)",
                                       "innotek",
                                       "Oracle",
                                       "VirtualBox",
        ...
        for (auto target : targets)
        {
            ...
            for (size_t i = 0; i <= bufLen - tlen; ++i)
            {
                if (memcmp(buf + i, target, tlen) == 0)
                {
                    ...
                    else if (!strcmp(target, "innotek") || !strcmp(target, "VirtualBox") || !strcmp(target, "vbox") || !strcmp(target, "VBOX") ||
                             !strcmp(target, "Oracle"))
                        brand = brands::VBOX;
    ...
    const bool all_digits = std::all_of(extract.cbegin(), extract.cend(), [](const char c) { return std::isdigit(c); });
    ...
    while ((entry = readdir(dir)) != nullptr)
    {
        ...
        for (const char* target : targets)
        {
            ...
            for (long j = 0; j <= file_size - static_cast<long>(targetLen); ++j)
            {
                ...
                {
                    const char* brand = nullptr;
                    if (strcmp(target, "Parallels Software International") == 0 || strcmp(target, "Parallels(R)") == 0)
                    {
                        brand = brands::PARALLELS;
                    }
                    else if (strcmp(target, "innotek") == 0 || strcmp(target, "Oracle") == 0 || strcmp(target, "VirtualBox") == 0 ||
                             strcmp(target, "vbox") == 0 || strcmp(target, "VBOX") == 0)
                    {
                        brand = brands::VBOX;
                    }
                    else if (strcmp(target, "VMware, Inc.") == 0 || strcmp(target, "VMware") == 0 || strcmp(target, "VMWARE") == 0)
                    {
                        brand = brands::VMWARE;
                    }
                    else if (strcmp(target, "QEMU"))
                    ...
                    else if (strcmp(target, "BOCHS") == 0 || strcmp(target, "BXPC") == 0)
                    {
                        brand = brands::BOCHS;
                    }
                    free(buffer);
...
{
    constexpr std::array<const char*, 21> known_ids = {{"AuthenticAMD", "CentaurHauls", "CyrixInstead", "GenuineIntel", "GenuineIotel", "TransmetaCPU",
                                                        "GenuineTMx86", "Geode by NSC", "NexGenDriven", "RiseRiseRise", "SiS SiS SiS ", "UMC UMC UMC ",
                                                        "Vortex86 SoC", "  Shanghai  ", "HygonGenuine", "Genuine  RDC", "E2K MACHINE",  "VIA VIA VIA ",
...
[[nodiscard]] static bool lspci()
{
    ...
    return false;
#else
    if (!((util::exists("/usr/bin/lspci")) || (util::exists("/bin/lspci")) || (util::exists("/usr/sbin/lspci"))))
    {
    ...
    if (pci_finder("QEMU PCIe Root port"))
    {
        return core::add(brands::QEMU);
    }
    if (pci_finder("QEMU XHCI Host Controller"))
    {
        return core::add(brands::QEMU);
    }
    if (pci_finder("QXL paravirtual graphic card"))
    {
        return core::add(brands::QEMU);
    }
    if (pci_finder("Virtio"))
    {
        return true;
    }            // could be used by a lot of brands, who knows
...
[[nodiscard]] static bool tpm()
{
    ...
    struct TbsContext
    {
        ...
        explicit TbsContext(const TBS_CONTEXT_PARAMS2& params) { Tbsi_Context_Create(reinterpret_cast<PCTBS_CONTEXT_PARAMS>(&params), &hContext); }
        ~TbsContext()
        {
            if (hContext)
            {
    ...
    if (Tbsip_Submit_Command(ctx.hContext, TBS_COMMAND_LOCALITY_ZERO, TBS_COMMAND_PRIORITY_NORMAL, cmd, static_cast<u32>(sizeof(cmd)), resp, &respSize) !=
            TBS_SUCCESS ||
        respSize < 27)
    {
        return false;
    ...
    const u32 manufacturerVal =
        (static_cast<u32>(resp[23]) << 24) | (static_cast<u32>(resp[24]) << 16) | (static_cast<u32>(resp[25]) << 8) | static_cast<u32>(resp[26]);
...
[[nodiscard]] static bool pci_vm_device_id()
{
    ...
    {
        debug("0x", std::setw(4), std::setfill('0'), std::hex, dev.vendor_id,
              "     | "
              "0x",
              std::setw(4), std::setfill('0'), dev.device_id, " | ", std::dec);
    }
    #endif
    ...
    for (const auto& dev : devices)
    {
        ...
        if (found)
        {
            debug("PCI_VM_DEVICE_ID: found vendor ID = ", "0x", std::setw(4), std::setfill('0'), std::hex, dev.vendor_id, " device ID = 0x", std::setw(4),
                  std::setfill('0'), std::hex, dev.device_id);
...
struct core
{
    ...
    [[nodiscard]] static inline bool is_disabled(const flagset& flags, const u8 flag_bit) noexcept { return (!flags.test(flag_bit)); }

    // same as above but for checking enabled flags
    [[nodiscard]] static inline bool is_enabled(const flagset& flags, const u8 flag_bit) noexcept { return (flags.test(flag_bit)); }
    ...
    static void flag_sanitizer(flagset& flags)
    {
        ...
        if (flags.test(NO_MEMO) || flags.test(HIGH_THRESHOLD) || flags.test(DYNAMIC) || flags.test(NULL_ARG) || flags.test(MULTIPLE))
        {
            generate_default(flags);
        }
        else
    ...
    static u16 run_all(const flagset& flags, const bool shortcut = false)
    {
        ...
        for (const auto& tmp : technique_table)
        {
            ...
            if ((technique_macro == VM::GPU_CAPABILITIES) || (technique_macro == VM::GPU_VM_STRINGS))
            {
                continue;
            }
        ...
        const std::vector<enum_flags> post_processed_techniques = {GPU_CAPABILITIES, GPU_VM_STRINGS};

        auto merge_scores = [&](const enum_flags a, const enum_flags b, const u8 new_score)
        {
            if (core::is_disabled(flags, a) || core::is_disabled(flags, b))
            {
                return;
            }
...
public:
    ...
    static void disable_flagset_manager(const flagset& flags) { disabled_flag_collector = flags; }

    static void disable_flag_manager(const enum_flags flag) { disabled_flag_collector.set(flag, false); }

    static void flag_manager(const enum_flags flag)
    {
        if ((flag == INVALID) || (flag > enum_size))
        {
    ...
    struct TestHandler
    {
        ...
        virtual void handle(const flagset& flags) { disable_flagset_manager(flags); }

        virtual void handle(const enum_flags flag) { flag_manager(flag); }
    };
    ...
    struct DisableTestHandler
    {
        ...
        virtual void disable_handle(const enum_flags flag) { disable_flag_manager(flag); }
    };
    ...
    struct TestBitsetHandler : public TestHandler
    {
        ...
        void handle(const flagset& flags) override { disable_flagset_manager(flags); }
    };
    ...
    struct TestUint8Handler : public TestHandler
    {
        ...
        void handle(const enum_flags flag) override { flag_manager(flag); }
    };
    ...
    struct DisableTestUint8Handler : public DisableTestHandler
    {
        ...
        void disable_handle(const enum_flags flag) override { disable_flag_manager(flag); }
    };
    ...
    static void handleArgs(First&& first, Rest&&... rest)
    {
        ...
        {
            const std::string msg = "Arguments must either be a std::bitset<" + std::to_string(static_cast<u32>(enum_size + 1)) +
    ...
    static void handle_disabled_args(First&& first, Rest&&... rest)
    {
        ...
        if (isType<flagset>(first))
        {
            throw std::invalid_argument(
                "Arguments must not contain VM::DEFAULT or VM::ALL, only technique flags are accepted (view the documentation for a full list)");
        }
        else if (isType<enum_flags>(first))
        {
 ...
 */
static bool check(const enum_flags flag_bit, const enum_flags memo_arg = NULL_ARG
...
)
{
    ...
    if ((flag_bit == NO_MEMO) || (flag_bit == HIGH_THRESHOLD) || (flag_bit == DYNAMIC) || (flag_bit == MULTIPLE))
    {
        throw_error("Flag argument must be a technique flag and not a settings flag");
    }

    if ((memo_arg != NO_MEMO) && (memo_arg != NULL_ARG))
    {
...
static std::string brand(Args... args)
{
    ...
    {
        if ((brands.count(a) > 0) && (brands.count(b) > 0))
        {
            brands.erase(a);
            brands.erase(b);
    ...
    {
        if ((brands.count(a) > 0) && (brands.count(b) > 0) && (brands.count(c) > 0))
        {
            brands.erase(a);
            brands.erase(b);
            brands.erase(c);
    ...
    std::sort(vec.begin(), vec.end(), [](const brand_element_t& a, const brand_element_t& b) { return a.second > b.second; });
 ...
 * @return void
 */
static void add_custom(const std::uint8_t percent, std::function<bool()> detection_func
// clang doesn't support std::source_location for some reason
#if (CPP >= 20 && !CLANG)
                       ,
                       const std::source_location& loc = std::source_location::current()
...
)
{
    ...
    core::custom_technique query{percent,
                                 // this fucking sucks
                                 static_cast<u16>(static_cast<int>(base_technique_count) + static_cast<int>(id)), detection_func};
...
[[nodiscard]] static std::string flag_to_string(const enum_flags flag)
{
    switch (flag)
    {
        ...
        /* GPL */ case QEMU_DIR:
            return "QEMU_DIR";
        /* GPL */ case POWER_CAPABILITIES:
            return "POWER_CAPABILITIES";
        /* GPL */ case SETUPAPI_DISK:
            return "SETUPAPI_DISK";
        case VM_PROCESSES:
            return "VM_PROCESSES";
        ...
        case MAC_SIP:
            return "MAC_SIP";
        case HKLM_REGISTRIES:
            return "HKLM_REGISTRIES";
        case VPC_INVALID:
            return "VPC_INVALID";
        case SIDT:
            return "SIDT";
        ...
        case VMWARE_PORT_MEM:
            return "VMWARE_PORT_MEM";
        case SMSW:
            return "SMSW";
        case MUTEX:
            return "MUTEX";
        case ODD_CPU_THREADS:
            return "ODD_CPU_THREADS";
        case INTEL_THREAD_MISMATCH:
            return "INTEL_THREAD_MISMATCH";
        case XEON_THREAD_MISMATCH:
            return "XEON_THREAD_MISMATCH";
        case NETTITUDE_VM_MEMORY:
            return "NETTITUDE_VM_MEMORY";
        case CUCKOO_DIR:
            return "CUCKOO_DIR";
        case CUCKOO_PIPE:
            return "CUCKOO_PIPE";
        case HYPERV_HOSTNAME:
            return "HYPERV_HOSTNAME";
        case GENERAL_HOSTNAME:
        ...
        case DISK_SERIAL:
            return "DISK_SERIAL";
        case PORT_CONNECTORS:
            return "PORT_CONNECTORS";
        case IVSHMEM:
            return "IVSHMEM";
        case GPU_VM_STRINGS:
            return "GPU_STRINGS";
        ...
        case PCI_VM_DEVICE_ID:
            return "PCI_VM_DEVICE_ID";
...
static std::vector<enum_flags> detected_enums(Args... args)
{
    ...
    {
        if ((flags.test(technique_enum)) && (check(static_cast<enum_flags>(technique_enum))))
        {
 ...
 */
static void modify_score(const enum_flags flag, const u8 percent
// clang doesn't support std::source_location for some reason
#if (CPP >= 20 && !CLANG)
                         ,
                         const std::source_location& loc = std::source_location::current()
...
static std::string type(Args... args)
{
    ...
    const std::map<std::string, const char*> type_table{
        ...
        {brands::HYPERV_ARTIFACT, "Unknown"},            // This refers to the type 1 hypervisor where Windows normally runs under, we put "Unknown" to clarify
                                                         // you're not running under a VM if this is detected
...
static std::string conclusion(Args... args)
{
    ...
    auto make_conclusion = [&](const std::string& category) -> std::string
    {
        ...
        if ((brand_tmp == brands::ACRN) || (brand_tmp == brands::ANUBIS) || (brand_tmp == brands::BSD_VMM) || (brand_tmp == brands::INTEL_HAXM) ||
            (brand_tmp == brands::APPLE_VZ) || (brand_tmp == brands::INTEL_KGT) || (brand_tmp == brands::POWERVM) || (brand_tmp == brands::OPENSTACK) ||
            (brand_tmp == brands::AWS_NITRO) || (brand_tmp == brands::OPENVZ) || (brand_tmp == brands::INTEL_TDX) || (brand_tmp == brands::AMD_SEV) ||
            (brand_tmp == brands::AMD_SEV_ES) || (brand_tmp == brands::AMD_SEV_SNP) || (brand_tmp == brands::NSJAIL) || (brand_tmp == brands::NULL_BRAND))
        {
            article = " an ";
        }
        else
...
std::map<const char*, VM::brand_score_t> VM::core::brand_scoreboard{{brands::VBOX, 0},
                                                                    {brands::VMWARE, 0},
