...
bool ManualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	if (pOldFileHeader->Machine != CURRENT_ARCH) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #001\n").c_str());
		//return false;
	}

	//ILog(XorStr("[ Deadly ] Starting Inject...\n").c_str());
	...
	if (!pTargetBase) {
		...
		char szErrorMsg[64];
		sprintf(szErrorMsg, XorStr("[ Deadly ] ERROR #03 (0x%X)").c_str(), GetLastError());
		ILog(szErrorMsg);
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pTargetBase, pSrcData, 0x1000, nullptr)) { //only first 0x1000 bytes for the header
		...
		//ILog(XorStr("[ Deadly ] ERROR #04").c_str());
	...
	for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
		if (pSectionHeader->SizeOfRawData) {
			if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSrcData + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr)) {
				...
				//ILog(XorStr("[ Deadly ] ERROR #05").c_str());
				VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
				return false;
			}
		}
	...
	if (!MappingDataAlloc) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #06").c_str());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, MappingDataAlloc, &data, sizeof(MANUAL_MAPPING_DATA), nullptr)) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #07").c_str());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
	...
	if (!pShellcode) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #08").c_str());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pShellcode, Shellcode, 0x1000, nullptr)) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #09").c_str());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
	...
	if (!hThread) {
		...
		//ILog(XorStr("[ Deadly ] ERROR #10").c_str());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
		return false;
	...
	while (!hCheck) {
		...
		if (hCheck == (HINSTANCE)0x404040) {
			...
			//ILog(XorStr("[ Deadly ] ERROR #11").c_str());
		}
	...
	if (ClearHeader) {
		if (!WriteProcessMemory(hProc, pTargetBase, emptyBuffer, 0x1000, nullptr)) {
			...
			//ILog(XorStr("[ Deadly ] ERROR #12").c_str());
		}
	}
	//END CLEAR PE HEAD
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				if ((SEHExceptionSupport ? 0 : strcmp((char*)pSectionHeader->Name, XorStr(".pdata").c_str()) == 0) ||
					strcmp((char*)pSectionHeader->Name, XorStr(".rsrc").c_str()) == 0 ||
					strcmp((char*)pSectionHeader->Name, XorStr(".reloc").c_str()) == 0) {
					...
					if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, emptyBuffer, pSectionHeader->Misc.VirtualSize, nullptr)) {
						...
						//ILog(XorStr("[ Deadly ] ERROR #13").c_str());
					}
				}
			}
		}
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				...
				if (VirtualProtectEx(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSectionHeader->Misc.VirtualSize, newP, &old)) {
					...
					//ILog(XorStr("[ Deadly ] ERROR #14").c_str());
				}
			}
		}
		DWORD old = 0;
...
void __stdcall Shellcode(MANUAL_MAPPING_DATA* pData) {
	...
	_DllMain(pBase, pData->fdwReasonParam, reinterpret_cast<void*>(4096));
