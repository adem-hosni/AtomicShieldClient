...
void CircleImage(ID3D11ShaderResourceView* user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1),
...
void CGUI::GUI::ApplyImage(int i)
{
    ...
    window->DrawList->AddRectFilledMultiColor({MIN.x, MIN.y}, {MAX.x - 160, MAX.y}, ImGui::GetColorU32(Colors::bg), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0),
...
bool CGUI::CUIElements::Button(const char* label, const char* text, float posX, float posY, float sizeX, float sizeY)
{
    ...
    it_anim->second.ButtonColor = ImVec4(ImLerp(it_anim->second.ButtonColor.x, target.x, time * 6), ImLerp(it_anim->second.ButtonColor.y, target.y, time * 6),
                                         ImLerp(it_anim->second.ButtonColor.z, target.z, time * 6), ImLerp(it_anim->second.ButtonColor.w, target.w, time * 6));
    it_anim->second.TextColor =
        ImVec4(ImLerp(it_anim->second.TextColor.x, targetText.x, time * 6), ImLerp(it_anim->second.TextColor.y, targetText.y, time * 6),
               ImLerp(it_anim->second.TextColor.z, targetText.z, time * 6), ImLerp(it_anim->second.TextColor.w, targetText.w, time * 6));
    it_anim->second.ShadowColor =
        ImVec4(ImLerp(it_anim->second.ShadowColor.x, ShadowColorTarget.x, time * 6), ImLerp(it_anim->second.ShadowColor.y, ShadowColorTarget.y, time * 6),
               ImLerp(it_anim->second.ShadowColor.z, ShadowColorTarget.z, time * 6), ImLerp(it_anim->second.ShadowColor.w, ShadowColorTarget.w / 2, time * 6));
...
bool CGUI::CUIElements::InputText(const char* label, const char* hint, float posX, float posY, float sizeX, static char buf[], size_t buf_size,
                                  ImGuiInputTextFlags flag)
{
    ...
    window->DrawList->AddShadowRect({MIN.x, MAX.y - 1}, MAX, ImGui::GetColorU32(it_anim->second.ShadowColor), it_anim->second.Shadow_size, {0, 0}, 0,
                                    items::rounding);
    ...
    it_anim->second.TextColor = ImVec4(ImLerp(it_anim->second.TextColor.x, TextTarget.x, time), ImLerp(it_anim->second.TextColor.y, TextTarget.y, time),
                                       ImLerp(it_anim->second.TextColor.z, TextTarget.z, time), ImLerp(it_anim->second.TextColor.w, TextTarget.w, time));
...
bool CGUI::CUIElements::IconButton(const char* label, const char* icon, float posX, float posY, int action)
{
    ...
    it_anim->second.TextColor = ImVec4(ImLerp(it_anim->second.TextColor.x, target.x, time * 6), ImLerp(it_anim->second.TextColor.y, target.y, time * 6),
                                       ImLerp(it_anim->second.TextColor.z, target.z, time * 6), ImLerp(it_anim->second.TextColor.w, target.w, time * 6));
...
void CGUI::CUIElements::AddBorders(ImVec2 pos)
{
    ...
    ImGui::GetWindowDrawList()->AddRect(ImVec2(1, 1), ImVec2(pos.x - 1, pos.y - 1), ImGui::GetColorU32(Colors::MainColor), window::rounding, ImDrawFlags_None,
                                        2.f);

    ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, ImVec2(1, 1),
                                                  ImVec2(pos.x / 4, pos.y / 3), ImGui::GetColorU32(Colors::MainColor), ImColor(24, 24, 24, 24));
}
...
void CGUI::CUI::BeforeLoop()
{
    ...
    if (images::valo == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, valo_p, sizeof(valo_p), nullptr, nullptr, &images::valo, 0);
    if (images::rust == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, rust_p, sizeof(rust_p), nullptr, nullptr, &images::rust, 0);
    if (images::fn == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, fn_p, sizeof(fn_p), nullptr, nullptr, &images::fn, 0);
    if (images::eft == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, EFT_p, sizeof(EFT_p), nullptr, nullptr, &images::eft, 0);
    if (images::mw == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, mw_p, sizeof(mw_p), nullptr, nullptr, &images::mw, 0);
    if (images::circle == nullptr)
        D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, circle_p, sizeof(circle_p), nullptr, nullptr, &images::circle, 0);

    if (fonts::Inter_Regular == nullptr)
        fonts::Inter_Regular = io.Fonts->AddFontFromMemoryTTF(inter_regular_p, sizeof(inter_regular_p), 16);
    if (fonts::Sansation_Light == nullptr)
        fonts::Sansation_Light = io.Fonts->AddFontFromMemoryTTF(sansation_light_p, sizeof(sansation_light_p), 13);
    if (fonts::Sansation_Regular == nullptr)
        fonts::Sansation_Regular = io.Fonts->AddFontFromMemoryTTF(sansation_regular_p, sizeof(sansation_regular_p), 25);
    if (fonts::Sansation_Bold == nullptr)
        fonts::Sansation_Bold = io.Fonts->AddFontFromMemoryTTF(sansation_bold_p, sizeof(sansation_bold_p), 28);
    if (fonts::FontAwesome == nullptr)
        fonts::FontAwesome = io.Fonts->AddFontFromMemoryCompressedTTF(fa6_solid_compressed_data, fa6_solid_compressed_size, 14.f, &icons_config, icons_ranges);
    ...
    g.begin("Main Window", window::size_max);
    {
        ...
        if (TAB == 1)
        {
            ...
            if (once)
            {
                ...
                strcat(szLoadingMessage, "Please wait...");
            }
            if (!strAgentPEBBuffer.empty() && !bInjected)
            {
                ...
                if (iProcessID)
                {
                    ...
                    if (hProcess)
                    {
                        ...
                        if (bInjected)
                            __fastfail(0);
                        printf("Result from dll injection: %d (0x%x)", bInjected, GetLastError());
            ...
            if (c.Tab("SCANNING", "Scanning...", NULL, 180, 28, ImGui::CalcTextSize("Scanning...").x, ImGui::CalcTextSize("Home").y, subtab == 0))
            {
                subtab = 0;
            }
            ...
            if (subtab == 0)
            {
                ...
                c.Text((window::size_max.x - ImGui::CalcTextSize(szLoadingMessage).x) / 2,
                       (window::size_max.y - ImGui::CalcTextSize(szLoadingMessage).y) / 1.9f, fonts::Inter_Regular, 17, szLoadingMessage, Colors::White);
                ...
                if (timer > 100)
                {
                    timer = 0;
                    alphaColor = 0;
                    ButtonPos = window::size_max.y + 5;
                    SelectablesPos = -335;
                    ParametersPos = window::size_max.x + 340;
                    subtab = 0;
                }
            }
        }
